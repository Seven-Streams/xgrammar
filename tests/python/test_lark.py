...
def _allowed_token_ids(matcher: xgr.GrammarMatcher) -> list:
...
def test_lark_lazy_mask_is_exit_only_after_commit() -> None:
    ...
    allowed = _allowed_token_ids(matcher)
    ...
    assert _allowed_token_ids(matcher) == [3]
...
def test_lark_lazy_root_anchored_occurrence() -> None:
    ...
    matcher = _lazy_matcher(grammar_obj)
    assert matcher.accept_token(0) and matcher.accept_token(2)
    assert _allowed_token_ids(matcher) == []
    _assert_grammar_language(grammar_obj, ["<a"], ["<ab", "<"])
...
def test_lark_lazy_rollback_reset_fork() -> None:
    ...
    matcher = _lazy_matcher(grammar_obj)
    assert matcher.accept_token(0) and matcher.accept_token(2)
    assert _allowed_token_ids(matcher) == [1]
    forked = matcher.fork()
    matcher.rollback(1)
    assert 2 in _allowed_token_ids(matcher)
    assert _allowed_token_ids(forked) == [1]
    matcher.reset()
