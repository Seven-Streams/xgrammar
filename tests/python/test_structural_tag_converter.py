...
def check_template_stag_with_grammar(
    ...
    **kwargs: List[Dict[str, str]],
    ...
    if structural_tag_format["type"] == "structural_tag":
        grammar = xgr.Grammar.apply_structural_tag_template(structural_tag_format, **kwargs)
    else:
        structural_tag = {"type": "structural_tag", "format": structural_tag_format}
        grammar = xgr.Grammar.apply_structural_tag_template(structural_tag, **kwargs)
...
def check_template_stag_with_instance(
    ...
    **kwargs: List[Dict[str, str]],
    ...
    if isinstance(structural_tag_format, StructuralTag) or (
        ...
        stag_grammar = xgr.Grammar.apply_structural_tag_template(structural_tag_format, **kwargs)
    else:
        structural_tag = {"type": "structural_tag", "format": structural_tag_format}
        stag_grammar = xgr.Grammar.apply_structural_tag_template(structural_tag, **kwargs)
    accepted = _is_grammar_accept_string(stag_grammar, instance, debug_print=debug_print)
    assert accepted == is_accepted
    if PROFILER_ON:
        profiler.profile_stag(structural_tag_format, instance)
...
def test_const_string_template_values_with_mingled_templates():
    ...
    with pytest.raises(Exception) as exc_info:
        xgr.Grammar.apply_structural_tag_template(
...
def test_part_json_schema_template_failure():
    ...
    with pytest.raises(Exception) as exc_info:
        xgr.Grammar.apply_structural_tag_template(structural_tag, types=types)
    expected_info = (
...
def test_part_qwen_template_failure():
    mingled_format = {"type": "qwen_xml_parameter", "json_schema": r"""{"type": {types[].value}}"""}
    structural_tag = {"type": "structural_tag", "format": mingled_format}
...
def test_part_regex_template_failure():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        xgr.Grammar.apply_structural_tag_template(structural_tag, patterns=patterns)
...
def test_part_grammar_template_failure():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        xgr.Grammar.apply_structural_tag_template(structural_tag, grammars=grammars)
    assert exc_info is not None
...
def test_no_parameter_error():
    ...
    grammars = [{"value": 'root ::= "a" | "b"'}, {"value": 'root ::= "c" | "d"'}]
    expected_error_info = (
        "Invalid structural tag error: No values provided for " "template placeholder: patterns"
    )
    ...
    (
        {
            "type": "sequence",
            "elements": [
                {"type": "const_string", "value": "{outter[].first}"},
                {"type": "or", "elements": [{"type": "const_string", "value": "{inner[].animal}"}]},
                {"type": "const_string", "value": "{outter[].symbol}"},
            ],
        },
        {
        ...
        r"""const_string ::= (("It is a "))
const_string_1 ::= (("dog"))
const_string_2 ::= (("cat"))
or ::= ((const_string_1) | (const_string_2))
const_string_3 ::= (("!"))
sequence ::= ((const_string or const_string_3))
const_string_4 ::= (("Is it a "))
const_string_5 ::= (("dog"))
const_string_6 ::= (("cat"))
or_1 ::= ((const_string_5) | (const_string_6))
const_string_7 ::= (("\?"))
sequence_1 ::= ((const_string_4 or_1 const_string_7))
...
def test_mingled_tag_template():
    ...
    inner = [{"animal": "dog"}, {"animal": "cat"}]
    with pytest.raises(Exception) as exc_info:
        xgr.Grammar.apply_structural_tag_template(structural_tag, outter=outter, inner=inner)
    expected_error_info = (
        "Invalid structural tag error: Multiple different placeholder names found in the tag "
        "format: 'outter' and 'inner'"
    )
    assert str(exc_info.value) == expected_error_info
...
def test_builtin_template_values(
    ...
    template_stag_format = xgr.structural_tag.get_builtin_template_structural_tag(
        builtin_format_type
    )
