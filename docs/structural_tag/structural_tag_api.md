...
By default (`"any_order": false`) properties must appear in their declared order and are fully
validated (no duplicate keys, all required keys present). With `"any_order": true`, the
**required** properties come first and the **non-required** ones (optional fields, plus
`additionalProperties` / `patternProperties` keys) follow, but the order *within* each group is
free. Only the **count** of required properties is checked — duplicate and missing keys are not
validated (e.g. with `required: ["a", "b"]`, two required entries are accepted even if `a` repeats
and `b` is missing). `minProperties` / `maxProperties` still bound the total count. It applies to
