...
class StateHash {
 public:
  size_t operator()(const ParserState& state) const {
    return HashCombine(state.sequence_id, state.element_id, state.sub_element_id);
  }
};
...
class StateHashChecker {
 public:
  size_t operator()(const ParserState& state) const {
    return HashCombine(state.sequence_id, state.element_id, state.input_pos, state.sub_element_id);
  }
};
