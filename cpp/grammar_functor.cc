...
class RuleInlinerImpl : public GrammarMutator {
 ...
 private:
  ...
  bool CheckIfRuleCanBeInlined(int32_t rule_id) {
    ...
    // Inlining a budgeted or lazy rule would erase the rule the attribute applies to.
    if (rule.max_tokens >= 0 || rule.is_lazy) {
      return false;
    }
    auto grammar_expr = base_grammar_->GetGrammarExpr(rule.body_expr_id);
...
int32_t RepetitionRangeExpanderImpl::HandleRepetitionRange(
  ...
  // Keep the reference to budgeted or lazy rules: replacing it with the rule's content would
  // erase the rule the attribute applies to.
